...
import {PinceptionServiceManager} from "../src/PinceptionServiceManager.sol";
...
// forge script script/PinceptionDeployer.s.sol:PinceptionDeployer --rpc-url $RPC_URL  --private-key $PRIVATE_KEY --broadcast -vvvv
contract PinceptionDeployer is Script, Utils {
    ...
    ProxyAdmin public pinceptionProxyAdmin;
    PauserRegistry public pinceptionPauserReg;
    
    ECDSAStakeRegistry public stakeRegistryProxy;
    ECDSAStakeRegistry public stakeRegistryImplementation;

    PinceptionServiceManager public pinceptionServiceManagerProxy;
    PinceptionServiceManager public pinceptionServiceManagerImplementation;
    ...
    function run() external {
        ...
        address pinceptionCommunityMultisig = msg.sender;
        address pinceptionPauser = msg.sender;
        ...
        _deployPinceptionContracts(
            delegationManager,
            avsDirectory,
            erc20MockStrategy,
            pinceptionCommunityMultisig,
            pinceptionPauser
        );
        vm.stopBroadcast();
    }
    ...
    function _deployPinceptionContracts(
        IDelegationManager delegationManager,
        IAVSDirectory avsDirectory,
        IStrategy strat,
        address pinceptionCommunityMultisig,
        address pinceptionPauser
    ) internal {
        ...
        pinceptionProxyAdmin = new ProxyAdmin();

        // deploy pauser registry
        {
            address[] memory pausers = new address[](2);
            pausers[0] = pinceptionPauser;
            pausers[1] = pinceptionCommunityMultisig;
            pinceptionPauserReg = new PauserRegistry(
                pausers,
                pinceptionCommunityMultisig
            );
        }
         ...
         */
        pinceptionServiceManagerProxy = PinceptionServiceManager(
            address(
                new TransparentUpgradeableProxy(
                    address(emptyContract),
                    address(pinceptionProxyAdmin),
                    ""
                )
            )
        );
        ...
        {   
            ...
            pinceptionProxyAdmin.upgradeAndCall(
        ...
        pinceptionServiceManagerImplementation = new PinceptionServiceManager(
        ...
        pinceptionProxyAdmin.upgrade(
            TransparentUpgradeableProxy(
                payable(address(pinceptionServiceManagerProxy))
            ),
            address(pinceptionServiceManagerImplementation)
        );
